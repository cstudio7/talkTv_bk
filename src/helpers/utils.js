 ...
 */
export const handleSuccessResponse = (data, message, res, status = 200) => res.status(status).json({
  status: 'success',
  message,
  data,
});
 ...
 * @return {object} response
 */
export const handleResponse = (data, message, res, status, statusCode) => res.status(statusCode).json({
  status,
  message,
  data,
});
